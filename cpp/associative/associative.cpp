...
Vector<T>::Vector(){
    it = 0;
    inc = 0; // сколько всего элементов есть в векторе
    count = 1; // если пользователь указал свое значение количества элементов
    data = new T[count]; // создание линейной памяти
    key = new string[count]; // массив ключей
}
...
template <typename T>
T& Vector<T>::operator[] (string _key) {
   bool global = false;
   int j = 0;

   for(j=0; j<count; j++){
       if (key[j] == _key) {break; global = true;}
   }
...
template <typename T>
int Vector<T>::end(){return count;}

template <typename T>
int& Vector<T>::iterator(){return it;}

template <typename T>
string Vector<T>::keyname(){return key[it];}

template <typename T>
T& Vector<T>::val(){return data[it];}


/*
template <typename T>
int& Vector<T>::iterator(){
   // iterator = 0;
...
template <typename T>
void Vector<T>::operator[] (string key) {
    cout << key << endl;
