...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function($) {'use strict';Object.defineProperty(exports,"__esModule",{value:true});var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var AppController=function(){/**
		 ...
		 */function AppController($rootScope,$scope,storageService){_classCallCheck(this,AppController);this.storage=storageService;this._newProject=angular.copy(storageService);window.clicked=false;window.clickX=0;window.clickY=0;$rootScope.onLoad=false;setTimeout(function(){$rootScope.onLoad=true;angular.element($('body')).scope().$apply();},2000);$('.mdl-layout__content').on({'mousemove':function mousemove(e){clicked&&updateScrollPos(e);},'mousedown':function mousedown(e){clicked=true;clickY=e.pageX;},'mouseup':function mouseup(){clicked=false;$('.mdl-layout__content').css('cursor','auto');}});var updateScrollPos=function updateScrollPos(e){$('.mdl-layout__content').css('cursor','move');$('.mdl-layout__content').scrollLeft($('.mdl-layout__content').scrollLeft()+(clickY-e.pageX));};// ctrl + s = сохранение проекта
	$(window).keypress(function(event){if(!(event.which==115&&event.ctrlKey)&&!(event.which==19))return true;$('.save-project').trigger('click');event.preventDefault();return false;});// ctrl + o = открытие проекта
	...
	this.storage.structure.item.push(angular.copy(storageService.base));}/**
	     ...
	     * @return {[type]} [description]
	     */},{key:'appQuit',value:function appQuit(){window.app.quit();}},{key:'newProject',value:function newProject(){var storageService=this.storage;var startService=this._newProject;angular.extend(storageService,startService);//angular.element($( 'body' )).scope().$apply();
	}}]);return AppController;}();var AppComponent=exports.AppComponent={template:__webpack_require__(11),controller:['$rootScope','$scope','storageService',AppController]};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(1)))
