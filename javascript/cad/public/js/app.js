...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function($) {'use strict';__webpack_require__(2);__webpack_require__(6);__webpack_require__(8);var _app=__webpack_require__(10);var _kernel=__webpack_require__(12);var _optionList=__webpack_require__(14);var _powerList=__webpack_require__(16);var _settlements=__webpack_require__(18);var _storageService=__webpack_require__(20);var _storageService2=_interopRequireDefault(_storageService);__webpack_require__(21);__webpack_require__(22);__webpack_require__(27);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}window.gauss=__webpack_require__(28);/*** Инициализация приложения */var app=angular.module('cad',['elif']);app.component('appComponent',_app.AppComponent);app.component('kernel',_kernel.KernelComponent);app.component('optionList',_optionList.OptionListComponent);app.component('powerList',_powerList.PowerListComponent);app.component('settlements',_settlements.SettlementsComponent);/*** Инициализация директив и сервисов */app.service('storageService',_storageService2.default);app.directive('dropdown',function(){var directive={restrict:'A',link:link};return directive;function link($scope,element,attrs){setTimeout(function(){return $(element).dropdown();},200);}});app.directive('collapsible',function(){var directive={restrict:'A',link:link};return directive;function link($scope,element,attrs){setTimeout(function(){return $(element).collapsible();},200);}});app.directive('closemenu',function(){var directive={restrict:'A',link:link};return directive;function link($scope,element,attrs){setTimeout(function(){$(element).on({'click':function click(e){$('.mdl-layout').addClass('is-small-screen');}});},200);}});
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function($) {'use strict';Object.defineProperty(exports,"__esModule",{value:true});var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var AppController=function(){/**
	  	 ...
	  	 */function AppController($rootScope,$scope,storageService){_classCallCheck(this,AppController);var self=this;this.storage=storageService;this.Math=window.Math;this.page={preprocess:true,process:false,postprocess:false};this._newProject=angular.copy(storageService);window.clicked=false;window.clickX=0;window.clickY=0;$rootScope.onLoad=false;setTimeout(function(){$rootScope.onLoad=true;angular.element($('body')).scope().$apply();},2000);$('.mdl-layout__content').on({'mousemove':function mousemove(e){clicked&&updateScrollPos(e);},'mousedown':function mousedown(e){clicked=true;clickY=e.pageX;},'mouseup':function mouseup(){clicked=false;$('.mdl-layout__content').css('cursor','auto');}});var updateScrollPos=function updateScrollPos(e){$('.mdl-layout__content').css('cursor','move');$('.mdl-layout__content').scrollLeft($('.mdl-layout__content').scrollLeft()+(clickY-e.pageX));};$(window).bind('keydown',function(event){if(event.ctrlKey||event.metaKey){switch(String.fromCharCode(event.which).toLowerCase()){case's':$('.save-project').trigger('click');event.preventDefault();break;case'o':$('.open-project').trigger('click');event.preventDefault();break;case'n':$('.new-project').trigger('click');event.preventDefault();break;}}});$('.mdl-layout__content').bind('mousewheel',function(e){if(!storageService.start)return;if(!self.page.preprocess)return;if(e.originalEvent.wheelDelta/120>0&&event.ctrlKey==true){if(storageService.ye<10){storageService.ye+=1;}}else if(e.originalEvent.wheelDelta/120<0&&event.ctrlKey==true){if(storageService.ye>5){storageService.ye-=1;}}angular.element($('body')).scope().$apply();e.preventDefault();return false;});$('input').on('focus',function(){self.reRender();});$(window).keydown(function(e){if(e.which==32){//space bar
	if(storageService.ye!=0){storageService.minimal=true;storageService.ye=0;angular.element($('body')).scope().$apply();}}});$(window).keyup(function(e){if(e.which==32){//space bar
	storageService.minimal=false;storageService.ye=10;angular.element($('body')).scope().$apply();}});}/**
	     * [reRender description]
	     * @return {[type]} [description]
	     */_createClass(AppController,[{key:'reRender',value:function reRender(){angular.element($('body')).scope().$apply();}/**
	     * [добавление стержня]
	     */},{key:'addKernelBlock',value:function addKernelBlock(){var storageService=this.storage;// первый старт приложения
	this.storage.start=true;// добавление первого стержня
	this.storage.structure.item.push(angular.copy(storageService.base));}/**
	     * Количество узлов
	     * @return {[type]} [description]
	     */},{key:'getNumberEdge',value:function getNumberEdge(){var storageService=this.storage;var countEdge=new Array(storageService.structure.item.length+1);return countEdge;}/**
	     * Открытие проекта
	     * @return {[type]} [description]
	     */},{key:'openProject',value:function openProject(){var $scope=this._$scope;var $rootScope=this._$rootScope;var storageService=this.storage;try{dialog.showOpenDialog({filters:[{name:'Файл проекта',extensions:['spс']}]},function(fileNames){if(fileNames===undefined)return;var fileName=fileNames[0];fs.readFile(fileName,'utf-8',function(err,data){try{var project=JSON.parse(data);if(project.hasOwnProperty('spc_version')){angular.extend(storageService,project);angular.element($('body')).scope().$apply();}else{dialog.showErrorBox("Данный файл не является файлом проекта",'Отсутствует строка spc_version');}}catch(err){dialog.showErrorBox("Данный файл не является файлом проекта",err.message);}});});}catch(e){alert(e);}}/**
	     * Сохранение проекта
	     * @return {[type]} [description]
	     */},{key:'saveProject',value:function saveProject(){var storageService=this.storage;var prj=JSON.stringify(storageService,null,"\t");try{dialog.showSaveDialog({filters:[{name:'Файл проекта',extensions:['spс']}]},function(fileName){if(fileName===undefined)return;fs.writeFile(fileName,prj,function(err){if(!err){dialog.showMessageBox({message:"Файл проекта успешно сохранен!",buttons:["OK"]});}else{dialog.showErrorBox("File Save Error",err.message);}});});}catch(e){alert(e);}}/**
	     * Завершение программы
	     * @return {[type]} [description]
	     */},{key:'appQuit',value:function appQuit(){window.app.quit();}},{key:'newProject',value:function newProject(){var storageService=this.storage;var startService=angular.copy(this._newProject);var page=this.page;for(var i in page){page[i]=false;}page.preprocess=true;angular.extend(storageService,startService);}/**
	     * [pageProcess description]
	     * @return {[type]} [description]
	     */},{key:'pageProcess',value:function pageProcess(){var page=this.page;for(var i in page){page[i]=false;}page.process=true;}},{key:'pagePreProcess',value:function pagePreProcess(){var page=this.page;for(var i in page){page[i]=false;}page.preprocess=true;}}]);return AppController;}();var AppComponent=exports.AppComponent={template:__webpack_require__(11),controller:['$rootScope','$scope','storageService',AppController]};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(1)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	'use strict';Object.defineProperty(exports,"__esModule",{value:true});var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var KernelController=function(){/**
		 ...
		 * @param  {Object} $rootScope     [глобальная область видимости приложения]
		 * @param  {Object} storageService [хранимая информация о приложении]
		 */function KernelController($rootScope,storageService){_classCallCheck(this,KernelController);this.storage=storageService;Array.prototype.insert=function(index,item){this.splice(index,0,item);};this.Math=window.Math;}/**
		 * Добавляем стержень
		 * @param {[type]} i [description]
		 */_createClass(KernelController,[{key:'addKernelBlock',value:function addKernelBlock(i){var storageService=this.storage;this.storage.structure.item.insert(i+1,angular.copy(storageService.base));}/**
		 * Удаляем стержень
		 * @param {[type]} i [description]
		 */},{key:'removeKernelBlock',value:function removeKernelBlock(i){this.storage.structure.item.splice(i,1);}/**
		 * Добавляем заделку справа
		 * [addSealingRight description]
		 */},{key:'addSealingRight',value:function addSealingRight(){var storageService=this.storage;storageService.structure.rightSealing=true;}/**
		 * Удаляем заделку справа
		 * [removeSealingRight description]
		 * @return {[type]} [description]
		 */},{key:'removeSealingRight',value:function removeSealingRight(){var storageService=this.storage;storageService.structure.rightSealing=false;}/**
		 * Добавляем заделку слева
		 * [addSealingLeft description]
		 */},{key:'addSealingLeft',value:function addSealingLeft(){var storageService=this.storage;storageService.structure.leftSealing=true;}/**
		 * Добавляем заделку слева
		 * [removeSealingLeft description]
		 * @return {[type]} [description]
		 */},{key:'removeSealingLeft',value:function removeSealingLeft(){var storageService=this.storage;storageService.structure.leftSealing=false;}/**
		 * Добавляем силу q
		 * [addPowerQ description]
		 * @param {[type]} i [description]
		 */},{key:'addPowerQ',value:function addPowerQ(i){var storageService=this.storage;storageService.structure.item[i].q=1;}/**
		 * Удаляем силу q
		 * [removePowerQ description]
		 * @param  {[type]} i [description]
		 * @return {[type]}   [description]
		 */},{key:'removePowerQ',value:function removePowerQ(i){var storageService=this.storage;storageService.structure.item[i].q=false;}/**
		 * Изменяем направление силы q
		 * @param  {[type]} i [description]
		 * @return {[type]}   [description]
		 */},{key:'changePowerQ',value:function changePowerQ(i){var storageService=this.storage;storageService.structure.item[i].q=-1*storageService.structure.item[i].q;}/**
		 * Добавляем силу F
		 * [addPowerF description]
		 * @param {[type]} i [description]
		 */},{key:'addPowerF',value:function addPowerF(i){var storageService=this.storage;storageService.structure.F[i]=1;}/**
		 * Добавляем силу F (противоположную)
		 * @param {[type]} i [description]
		 */},{key:'addPowerLeftF',value:function addPowerLeftF(i){var storageService=this.storage;storageService.structure.F[i]=-1;}},{key:'removePowerF',value:function removePowerF(i){var storageService=this.storage;storageService.structure.F[i]=null;}}]);return KernelController;}();var KernelComponent=exports.KernelComponent={bindings:{kid:'=',index:'=',nextIndex:'=',structure:'=',storage:'='},template:__webpack_require__(13),controller:['$rootScope','storageService',KernelController]};

/***/ },
/* 13 */
/***/ function(module, exports) {

	module.exports = "<div class=\"block idblock-{{$ctrl.index}}\" \n     style=\"\n        height: {{ ($ctrl.structure[$ctrl.index].A / $ctrl.structure[$ctrl.index].L) * $ctrl.storage.ye }}px;\n        width: {{ $ctrl.structure[$ctrl.index].L * ($ctrl.storage.ye < 5 ? 10 : $ctrl.storage.ye) }}px;\n     \"\n     ng-class=\"{ 'arrow' : $ctrl.structure[$ctrl.index].q !== false ? 1 : 0, 'rotate' : $ctrl.structure[$ctrl.index].q < 0 ? 1:0 }\">\n\n\t<div class=\"nameing\">{{$ctrl.index+1}}\n    <div class=\"before\">{{$ctrl.structure[$ctrl.index].L / $ctrl.storage.base.L}}L</div>\n    <div class=\"after\">{{$ctrl.structure[$ctrl.index].A / $ctrl.storage.base.A}}A</div>\n  </div>\n\n  <div class=\"nameing-bottom\">\n    <div class=\"before\" ng-if=\"$ctrl.storage.structure.F[$ctrl.index]\">{{ $ctrl.Math.abs($ctrl.storage.structure.F[$ctrl.index] / $ctrl.storage.kF) }}F</div>\n    <div class=\"after\" ng-if=\"$ctrl.structure[$ctrl.index].q\">{{ $ctrl.Math.abs($ctrl.structure[$ctrl.index].q / $ctrl.storage.kq) }}q</div>\n  </div>\n\t\n\t<div class=\"block-btn addRight\" data-activates='dropdown-right-{{$ctrl.index}}' dropdown>+</div>\n\t<div class=\"block-btn addLeft\" data-activates='dropdown-left-{{$ctrl.index}}' dropdown>+</div>\n\t<div class=\"block-btn addCenter\" data-activates='dropdown-center-{{$ctrl.index}}' dropdown>+</div>\n\n  <div class=\"edge edgeLeft\">{{$ctrl.index+1}}</div>\n  <div class=\"edge edgeRight\">{{$ctrl.index+2}}</div>\n\n  <div class=\"rightPower\" ng-if=\"$ctrl.storage.structure.F[$ctrl.index] !== null && $ctrl.storage.structure.F[$ctrl.index] > 0\"></div>\n  <div class=\"leftPower\" ng-if=\"$ctrl.storage.structure.F[$ctrl.index+1] !== null && $ctrl.storage.structure.F[$ctrl.index+1] < 0\"></div>\n\n\t<div ng-if=\"(($ctrl.index+1) == 1)\" class=\"sealingLeft\" \n       ng-class=\"{ 'show-block' :  $ctrl.storage.structure.leftSealing?1:0 }\" \n       style=\"transform: rotateY(180deg);\"></div>\n\n\t<div ng-if=\"(($ctrl.index+1) == $ctrl.structure.length)\" \n       class=\"sealingRight\" \n       ng-class=\"{ 'show-block' :  $ctrl.storage.structure.rightSealing?1:0 }\"></div>\n\t\n\t<ul id='dropdown-right-{{$ctrl.index}}' class='dropdown-content'>\n    \t<li ng-click=\"$ctrl.addKernelBlock($ctrl.index)\">\n    \t\t<a class=\"addBlockToRight\" ref=\"#!\">Добавить стержень</a>\n    \t\t<div class=\"divider\"></div>\n    \t</li>\n\n    \t<li ng-if=\"($ctrl.index+1) == $ctrl.structure.length && !$ctrl.storage.structure.rightSealing\" ng-click=\"$ctrl.addSealingRight()\">\n    \t\t<a class=\"sealing addSealingRight\" href=\"#!\">Добавить заделку</a>\n    \t\t<div class=\"divider\"></div>\n    \t</li>\n\n      <li ng-if=\"($ctrl.index+1) == $ctrl.structure.length && $ctrl.storage.structure.rightSealing\" ng-click=\"$ctrl.removeSealingRight()\">\n        <a class=\"sealing addSealingRight\" href=\"#!\">Удалить заделку</a>\n        <div class=\"divider\"></div>\n      </li>\n\n    \t<li ng-if=\"!(($ctrl.index+1) == $ctrl.structure.length) && !$ctrl.storage.structure.F[$ctrl.index+1]\" ng-click=\"$ctrl.addPowerF($ctrl.index+1)\">\n    \t\t<a class=\"\" href=\"#!\">Добавить усилие (F) вправо от узла </a>\n    \t</li>\n\n      <li ng-if=\"$ctrl.storage.structure.F[$ctrl.index+1]\" ng-click=\"$ctrl.removePowerF($ctrl.index+1)\">\n        <a class=\"\" href=\"#!\">Удалить усилие (F) </a>\n      </li>\n\n  \t</ul>\n\t\n\t  <ul id='dropdown-left-{{$ctrl.index}}' class='dropdown-content'>\n    \t<li ng-if=\"($ctrl.index+1 == 1) && !$ctrl.storage.structure.leftSealing\" ng-click=\"$ctrl.addSealingLeft()\">\n    \t\t<a class=\"sealing addSealingLeft\" href=\"#!\">Добавить заделку</a>\n    \t\t<div class=\"divider\"></div>\n    \t</li>\n\n      <li ng-if=\"($ctrl.index+1 == 1) && $ctrl.storage.structure.leftSealing\" ng-click=\"$ctrl.removeSealingLeft()\">\n        <a class=\"sealing addSealingLeft\" href=\"#!\">Удалить заделку</a>\n        <div class=\"divider\"></div>\n      </li>\n\n    \t<li ng-if=\"!(($ctrl.index+1) == 1) && !$ctrl.storage.structure.F[$ctrl.index]\" ng-click=\"$ctrl.addPowerLeftF($ctrl.index)\">\n    \t\t<a class=\"\" href=\"#!\">Добавить усилие (F) влево от узла</a>\n    \t</li>\n\n      <li ng-if=\"$ctrl.storage.structure.F[$ctrl.index]\" ng-click=\"$ctrl.removePowerF($ctrl.index)\">\n        <a class=\"\" href=\"#!\">Удалить усилие (F)</a>\n      </li>\n\n  \t</ul>\n\n\t\t<ul id='dropdown-center-{{$ctrl.index}}' class='dropdown-content'>\n\t    \t<li ng-click=\"$ctrl.addPowerQ($ctrl.index)\" ng-if=\"$ctrl.structure[$ctrl.index].q === false\">\n\t    \t\t<a class=\"\" href=\"#!\">Добавить нагрузку (q)</a>\n\t    \t</li>\n\n        <li ng-click=\"$ctrl.removePowerQ($ctrl.index)\" ng-if=\"$ctrl.structure[$ctrl.index].q !== false\">\n          <a class=\"\" href=\"#!\">Удалить нагрузку (q)</a>\n        </li>\n\n        <li ng-click=\"$ctrl.changePowerQ($ctrl.index)\" ng-if=\"$ctrl.structure[$ctrl.index].q !== false\">\n          <a class=\"\" href=\"#!\">Изменить направление нагрузки (q)</a>\n        </li>\n\n        <li ng-click=\"$ctrl.removeKernelBlock($ctrl.index)\">\n          <a class=\"\" href=\"#!\">Удалить стержень</a>\n        </li>\n\n\t  \t</ul>\n\t\n</div>";

/***/ },
/* 14 */
/***/ function(module, exports, __webpack_require__) {

	'use strict';Object.defineProperty(exports,"__esModule",{value:true});function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var OptionListController=/**
		 * [инициация структуры приложения]
		 * @param  {Object} $rootScope     [глобальная область видимости приложения]
		 * @param  {Object} storageService [хранимая информация о приложении]
		 */function OptionListController($rootScope,storageService){_classCallCheck(this,OptionListController);this.storage=storageService;};var OptionListComponent=exports.OptionListComponent={bindings:{kid:'=',index:'=',nextIndex:'=',structure:'=',storage:'='},template:__webpack_require__(15),controller:['$rootScope','storageService',OptionListController]};

/***/ },
/* 15 */
/***/ function(module, exports) {

	module.exports = "<ul class=\"collapsible\" data-collapsible=\"accordion\" collapsible>\n\n    <li>\n        \n        <div class=\"collapsible-header active\">\n         <i class=\"Tiny material-icons clearfix\">view_module</i>\n         <p> параметры стержня ({{$ctrl.index + 1}})</p>\n        </div>\n        \n        <div class=\"collapsible-body\">\n\n            <div class=\"row\">\n\n                <div class=\"col s4\">\n            \n                    <div class=\"\">\n                         <input placeholder=\"30\"\n                                type=\"number\" \n                                ng-model=\"$ctrl.structure.item[$ctrl.index].L\" \n                                value=\"{{$ctrl.structure.item[$ctrl.index].L}}\" \n                                ng-minlength=\"0\"\n                                min=\"0\"\n                                class=\"validate Lchange\">\n\n                         <label>Длинна (L, м)</label>\n                    </div>\n\n                </div>\n\n                <div class=\"col s2\">\n                    <div class=\"\">\n                         <input placeholder=\"3L\"\n                                type=\"text\" \n                                value=\"{{$ctrl.structure.item[$ctrl.index].L / $ctrl.storage.base.L}}L\"\n                                disabled=\"disabled\" \n                                class=\"validate Lchange\">\n\n                         <label>kL</label>\n                    </div>\n\n                </div>\n\n                <div class=\"col s4\">\n\n                    <div class=\"\">\n                         <input placeholder=\"300\"  \n                                type=\"number\" \n                                ng-model=\"$ctrl.structure.item[$ctrl.index].A\" \n                                value=\"{{$ctrl.structure.item[$ctrl.index].A}}\" \n                                ng-minlength=\"0\"\n                                min=\"0\"\n                                class=\"validate Achange\">\n\n                         <label>Площадь (A, м<sup>2</sup>)</label>\n                    </div>\n\n                </div>\n\n                <div class=\"col s2\">\n                    <div class=\"\">\n                         <input placeholder=\"3A\"\n                                type=\"text\" \n                                value=\"{{$ctrl.structure.item[$ctrl.index].A / $ctrl.storage.base.A}}A\"\n                                disabled=\"disabled\" \n                                class=\"validate Lchange\">\n\n                         <label>kA</label>\n                    </div>\n\n                </div>\n\n                <div class=\"col s6\">\n                \n                    <div class=\"\">\n                         <input placeholder=\"10e5\"\n                                type=\"text\" \n                                ng-model=\"$ctrl.structure.item[$ctrl.index].E\" \n                                value=\"{{($ctrl.structure.item[$ctrl.index].E).toExponential()}}\" \n                                ng-minlength=\"0\"\n                                min=\"0\"\n                                class=\"validate Lchange\">\n\n                         <label>Модуль упругости (E, Па)</label>\n                    </div>\n\n                </div>\n\n                <div class=\"col s6\">\n                \n                    <div class=\"\">\n                         <input placeholder=\"\"\n                                type=\"number\" \n                                ng-model=\"$ctrl.structure.item[$ctrl.index].sigma\" \n                                value=\"{{$ctrl.structure.item[$ctrl.index].sigma}}\" \n                                ng-minlength=\"0\"\n                                min=\"0\"\n                                class=\"validate Lchange\">\n\n                         <label>[&sigma;] (Па)</label>\n                    </div>\n\n                </div>\n              \n            </div>\n\n\n        </div>\n\n      </li>\n  </ul>";

/***/ },
/* 16 */
/***/ function(module, exports, __webpack_require__) {

	'use strict';Object.defineProperty(exports,"__esModule",{value:true});function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var PowerListController=/**
		 * [инициация структуры приложения]
		 * @param  {Object} $rootScope     [глобальная область видимости приложения]
		 * @param  {Object} storageService [хранимая информация о приложении]
		 */function PowerListController($rootScope,storageService){_classCallCheck(this,PowerListController);this.storage=storageService;this.Math=window.Math;};var PowerListComponent=exports.PowerListComponent={bindings:{kid:'=',index:'=',nextIndex:'=',structure:'=',storage:'='},template:__webpack_require__(17),controller:['$rootScope','storageService',PowerListController]};

/***/ },
/* 17 */
/***/ function(module, exports) {

	module.exports = "<ul class=\"collapsible\" data-collapsible=\"accordion\" collapsible \n    ng-if=\"$ctrl.structure.item[$ctrl.index].q || $ctrl.structure.item[$ctrl.index-1].q || $ctrl.storage.structure.F[$ctrl.index]\">\n\n    <li>\n        \n        <div class=\"collapsible-header active\">\n         <i class=\"Tiny material-icons clearfix\">compare_arrows</i>\n         <p> параметры узла ({{$ctrl.index + 1}})</p>\n        </div>\n        \n        <div class=\"collapsible-body\">\n\n            <div class=\"row\">\n                <div class=\"col s4\">\n\n                    <div ng-if=\"!$ctrl.structure.item[$ctrl.index].q && !$ctrl.structure.item[$ctrl.index-1].q\">\n                         <input placeholder=\"0\"\n                                type=\"number\"\n                                value=\"0\"\n                                disabled=\"disabled\"\n                                class=\"validate Lchange\">\n\n                         <label>q (Н/м)</label>\n                    </div>\n\n                    <div ng-if=\"$ctrl.structure.item[$ctrl.index-1].q\">\n                         <input placeholder=\"30\"\n                                type=\"number\"\n                                ng-model=\"$ctrl.structure.item[$ctrl.index-1].q\"\n                                value=\"{{$ctrl.structure.item[$ctrl.index-1].q}}\"\n                                class=\"validate Lchange\">\n\n                         <label>q (Н/м)</label>\n                    </div>\n\n                    <div ng-if=\"$ctrl.structure.item[$ctrl.index].q\">\n                         <input placeholder=\"30\"\n                                type=\"number\"\n                                ng-model=\"$ctrl.structure.item[$ctrl.index].q\"\n                                value=\"{{$ctrl.structure.item[$ctrl.index].q}}\"\n                                class=\"validate Lchange\">\n\n                         <label>q (Н/м)</label>\n                    </div>\n\n                </div>\n\n                <div class=\"col s2\">\n                    <div ng-if=\"$ctrl.structure.item[$ctrl.index-1].q\">\n                         <input placeholder=\"3q\"\n                                type=\"text\" \n                                value=\"{{ $ctrl.Math.abs($ctrl.structure.item[$ctrl.index-1].q / $ctrl.storage.kq) }}q\"\n                                disabled=\"disabled\" \n                                class=\"validate Lchange\">\n\n                         <label>kq</label>\n                    </div>\n\n                    <div ng-if=\"$ctrl.structure.item[$ctrl.index].q\">\n                         <input placeholder=\"3q\"\n                                type=\"text\" \n                                value=\"{{ $ctrl.Math.abs($ctrl.structure.item[$ctrl.index].q / $ctrl.storage.kq) }}q\"\n                                disabled=\"disabled\" \n                                class=\"validate Lchange\">\n\n                         <label>kq</label>\n                    </div>\n\n                </div>\n\n                <div class=\"col s4\">\n\n                    <div class=\"\">\n                         <input placeholder=\"300\"  \n                                type=\"number\"\n                                ng-model=\"$ctrl.storage.structure.F[$ctrl.index]\"\n                                value=\"{{$ctrl.storage.structure.F[$ctrl.index] || 0}}\"\n                                class=\"validate Achange\">\n\n                         <label>F (Н)</label>\n                    </div>\n\n                </div>\n\n                <div class=\"col s2\">\n                    <div class=\"\">\n                         <input placeholder=\"3F\"\n                                type=\"text\" \n                                value=\"{{ $ctrl.Math.abs($ctrl.storage.structure.F[$ctrl.index] / $ctrl.storage.kF) || 0 }}F\"\n                                disabled=\"disabled\" \n                                class=\"validate Lchange\">\n\n                         <label>kF</label>\n                    </div>\n\n                </div>\n              \n            </div>\n\n\n        </div>\n\n      </li>\n  </ul>";

/***/ },
/* 18 */
/***/ function(module, exports, __webpack_require__) {

	"use strict";Object.defineProperty(exports,"__esModule",{value:true});var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var SettlementsController=function(){/**
		 * [инициация структуры приложения]
		 * @param  {Object} $rootScope     [глобальная область видимости приложения]
		 * @param  {Object} storageService [хранимая информация о приложении]
		 */function SettlementsController($rootScope,$scope,storageService){_classCallCheck(this,SettlementsController);this.storage=storageService;// создаем матрицу реакций А
	this.tempA=[];var len=storageService.structure.item.length+1;for(var i=0;i<len;i++){this.tempA[i]=[];for(var j=0;j<len;j++){this.tempA[i][j]=0;}}this.tempQ=new Array(len).fill(0);;for(var _i=0,setter=false;_i<len;_i++){var F=storageService.structure.F[_i];var L=storageService.structure.item[_i==len-1?_i-1:_i].L;var q=storageService.structure.item[_i==len-1?_i-1:_i].q;var kF=storageService.kF;var kq=storageService.kq;var kL=storageService.base.L;if(typeof F=="number"){setter=true;this.tempQ[_i]="";this.tempQ[_i]+=F/kF+"F";}// проверяем для первого узла
	if(_i==0||_i==len-1){if(q!=false){if(!setter)this.tempQ[_i]="";else{if(q>0)this.tempQ[_i]+=" + ";}this.tempQ[_i]+=q/kq+"*"+L/kL+"qL/2";}}else{var q1=storageService.structure.item[_i-1].q;var L1=storageService.structure.item[_i-1].L;var q2=storageService.structure.item[_i].q;var L2=storageService.structure.item[_i].L;if(!setter)this.tempQ[_i]="";else{if(q1>0)this.tempQ[_i]+=" + ";}if(q1!=false){this.tempQ[_i]+=q1/kq+"*"+L1/kL+"qL/2";setter=true;}if(!setter)this.tempQ[_i]="";else{if(q2>0)this.tempQ[_i]+=" + ";}if(q2!=false){this.tempQ[_i]+=q2/kq+"*"+L2/kL+"qL/2";setter=true;}}setter=false;}this.A=[];this.Q=[];this.resultA=[];this.resultQ=[];this.resultDelta=[];}/**
		 * [getNumberEdgeTo description]
		 * @return {[type]} [description]
		 */_createClass(SettlementsController,[{key:"getNumberEdgeTo",value:function getNumberEdgeTo(){var storageService=this.storage;var len=storageService.structure.item.length+1;var temp=[];for(var i=0;i<len;i++){temp.push(1+i);}return temp;}},{key:"appendToTempA",value:function appendToTempA(i,j,p,type){var storageService=this.storage;i--;j--;if(type=="K11"){var kA=storageService.structure.item[p-1].A/storageService.base.A;var kL=storageService.structure.item[p-1].L/storageService.base.L;this.tempA[i][j]=" "+kA+" EA / "+kL+"L";}if(type=="K12"||type=="K21"){var _kA=storageService.structure.item[p-1].A/storageService.base.A;var _kL=storageService.structure.item[p-1].L/storageService.base.L;this.tempA[i][j]="- "+_kA+" EA / "+_kL+"L";}if(type=="K22+K11"){var kA1=storageService.structure.item[p-1].A/storageService.base.A;var kL1=storageService.structure.item[p-1].L/storageService.base.L;var kA2=storageService.structure.item[p].A/storageService.base.A;var kL2=storageService.structure.item[p].L/storageService.base.L;this.tempA[i][j]="("+kA1+" EA / "+kL1+"L) + ("+kA2+" EA / "+kL2+"L)";}}/**
		 * [tempInto description]
		 * @return {[type]} [description]
		 */},{key:"tempInto",value:function tempInto(){var storageService=this.storage;var len=storageService.structure.item.length+1;// обнуление заделки матрица A
	this.A=angular.copy(this.tempA);if(storageService.structure.leftSealing){this.A[0][0]=1;this.A[1][0]=0;for(var i=1;i<len;i++){this.A[0][i]=0;}}if(storageService.structure.rightSealing){for(var _i2=0;_i2<len;_i2++){this.A[len-1][_i2]=0;}this.A[len-1][len-1]=1;this.A[len-2][len-1]=0;}// обнуление заделки вектор b
	this.Q=angular.copy(this.tempQ);if(storageService.structure.leftSealing)this.Q[0]=0;if(storageService.structure.rightSealing)this.Q[len-1]=0;this.resultA=angular.copy(this.A);this.resultQ=angular.copy(this.Q);// вычисление коэффициентов
	for(var _i3=0;_i3<len;_i3++){for(var j=0;j<len;j++){if(typeof this.resultA[_i3][j]=="string"){this.resultA[_i3][j]=+eval(this.resultA[_i3][j].replace(/F|q|L|E|A/g,'')).toFixed(2);}}}for(var _i4=0;_i4<len;_i4++){if(typeof this.resultQ[_i4]=="string"){this.resultQ[_i4]=+eval(this.resultQ[_i4].replace(/F|q|L|E|A/g,'')).toFixed(2);}}console.log(this.resultA,this.resultQ);var $A=angular.copy(this.resultA);var $x=angular.copy(this.resultQ);this.resultDelta=gauss($A,$x);}},{key:"gaussRound",value:function gaussRound(num,decimalPlaces){var d=decimalPlaces||0,m=Math.pow(10,d),n=+(d?num*m:num).toFixed(8),i=Math.floor(n),f=n-i,e=1e-8,r=f>0.5-e&&f<0.5+e?i%2==0?i:i+1:Math.round(n);return d?r/m:r;}},{key:"ceil",value:function ceil(i){return Math.ceil(i);}}]);return SettlementsController;}();var SettlementsComponent=exports.SettlementsComponent={template:__webpack_require__(19),controller:['$rootScope','$scope','storageService',SettlementsController]};

/***/ },
/* 19 */
/***/ function(module, exports) {

	module.exports = "<div class=\"page-process\">\n\t<p><b>Уравнение равновесия всех узлов:</b></p>\n\t<p>[A]{&Delta;} = {b}</p>\n\t<p>[A] - глобальная матрица реакций</p>\n\t<p>Кол-во узлов: {{ $ctrl.getNumberEdgeTo().length }}</p>\n\n\t<table>\n\t\t<tr ng-repeat=\"i in $ctrl.getNumberEdgeTo() track by $index\">\n\t\t\t<td ng-repeat=\"j in $ctrl.getNumberEdgeTo() track by $index\">\n\t\t\t\t\n\t\t\t\t\t<span ng-if=\" (i == 1) && ( i == j || (i+1) == j ) \" >\n\t\t\t\t\t\t<span  ng-if=\" j % 2\" >\n\t\t\t\t\t\t\tK<sup>{{j}}</sup><sub>11</sub>\n\t\t\t\t\t\t\t{{ $ctrl.appendToTempA(i, j, j, \"K11\") }}\n\t\t\t\t\t\t</span>\n\t\t\t\t\t\t<span  ng-if=\" !(j % 2) \">\n\t\t\t\t\t\t\tK<sup>{{j-1}}</sup><sub>12</sub>\n\t\t\t\t\t\t\t{{ $ctrl.appendToTempA(i, j, j-1, \"K12\") }}\n\t\t\t\t\t\t</span>\n\t\t\t\t\t</span>\n\n\t\t\t\t\t<span ng-else-if=\" (i != 1 && i != $ctrl.getNumberEdgeTo().length ) && ( (i-1) == j || i == j || (i+1) == j )\" >\n\t\t\t\t\t\t<span  ng-if=\" (i-1) == j \">\n\t\t\t\t\t\t\tK<sup>{{i-1}}</sup><sub>21</sub>\n\t\t\t\t\t\t\t{{ $ctrl.appendToTempA(i, j, i-1, \"K21\") }}\n\t\t\t\t\t\t</span>\n\t\t\t\t\t\t<span  ng-if=\" i == j \">\n\t\t\t\t\t\t\tK<sup>{{i-1}}</sup><sub>22</sub> + K<sup>{{i}}</sup><sub>11</sub>\n\t\t\t\t\t\t\t{{ $ctrl.appendToTempA(i, j, i-1, \"K22+K11\") }}\n\t\t\t\t\t\t</span>\n\t\t\t\t\t\t<span  ng-if=\" (i+1) == j \">\n\t\t\t\t\t\t\tK<sup>{{i}}</sup><sub>12</sub>\n\t\t\t\t\t\t\t{{ $ctrl.appendToTempA(i, j, i, \"K12\") }}\n\t\t\t\t\t\t</span>\n\t\t\t\t\t</span>\n\n\t\t\t\t\t<span ng-else-if=\" (i == $ctrl.getNumberEdgeTo().length) && ( (i-1) == j || i == j ) \" >\n\t\t\t\t\t\t<span  ng-if=\"(i-1) == j\">\n\t\t\t\t\t\t\tK<sup>{{i}}</sup><sub>21</sub>\n\t\t\t\t\t\t\t{{ $ctrl.appendToTempA(i, j, i-1, \"K21\") }}\n\t\t\t\t\t\t</span>\n\n\t\t\t\t\t\t<span  ng-if=\" i == j \">\n\t\t\t\t\t\t\tK<sup>{{i}}</sup><sub>11</sub>\n\t\t\t\t\t\t\t{{ $ctrl.appendToTempA(i, j, i-1, \"K11\") }}\n\t\t\t\t\t\t</span>\n\t\t\t\t\t</span>\n\n\t\t\t\t\t<span ng-else>0</span>\n\t\t\n\t\t\t</td>\n\t\t</tr>\n\t</table>\n\n\t<table>\n\t\t<tr>\n\t\t\t<td>K<sup>p</sup><sub>11</sub></td>\n\t\t\t<td class=\"slim-right\">K<sup>p</sup><sub>12</sub></td>\n\t\t\t<td rowspan=\"2\">=</td>\n\t\t\t<td class=\"slim-left\">E<sub>p</sub>A<sub>p</sub>/L<sub>p</sub></td>\n\t\t\t<td>-E<sub>p</sub>A<sub>p</sub>/L<sub>p</sub></td>\n\t\t</tr>\n\n\t\t<tr>\n\t\t\t<td>K<sup>p</sup><sub>21</sub></td>\n\t\t\t<td class=\"slim-right\">K<sup>p</sup><sub>22</sub></td>\n\t\n\t\t\t<td class=\"slim-left\">-E<sub>p</sub>A<sub>p</sub>/L<sub>p</sub></td>\n\t\t\t<td>E<sub>p</sub>A<sub>p</sub>/L<sub>p</sub></td>\n\t\t</tr>\n\n\t</table>\n\n\t<br>\n\t<p>В соответствии с методом перемещений составляем соответствующую систему линейных алгебраических уравнений: </p>\n\n\t<table>\n\t\t<tr ng-repeat=\"i in $ctrl.getNumberEdgeTo() track by $index\">\n\t\t\t<td ng-repeat=\"j in $ctrl.getNumberEdgeTo() track by $index\">\n\t\t\t\t\n\n\t\t\t\t{{$ctrl.tempA[i-1][j-1]}}\n\t\t\t</td>\n\t\t\t\n\t\t\t<td class=\"slim-left slim-right\" ng-if=\" i == 1 \" rowspan=\"{{ $ctrl.getNumberEdgeTo().length }}\">Х</td>\n\t\t\t\n\t\t\t<td class=\"slim-left slim-right\">\n\t\t\t\t&Delta; <sub>{{i}}</sub>\n\t\t\t</td>\n\t\t\t\n\t\t\t<td class=\"slim-left slim-right\" ng-if=\" i == 1 \" rowspan=\"{{ $ctrl.getNumberEdgeTo().length }}\">=</td>\n\n\t\t\t<td>\n\t\t\t\t{{$ctrl.tempQ[i-1]}}\n\t\t\t</td>\n\n\t\t</tr>\n\t</table>\n\t\n\t<br>\n\t<p>Учитываем кинематические граничные условия &Delta;<sub>1</sub> = &Delta;<sub>{{$ctrl.getNumberEdgeTo().length}}</sub> </p>\n\n\t{{ $ctrl.tempInto() }}\n\n\t<table>\n\t\t<tr ng-repeat=\"i in $ctrl.getNumberEdgeTo() track by $index\">\n\t\t\t<td ng-repeat=\"j in $ctrl.getNumberEdgeTo() track by $index\">\n\t\t\t\t\n\n\t\t\t\t{{$ctrl.A[i-1][j-1]}}\n\t\t\t</td>\n\t\t\t\n\t\t\t<td class=\"slim-left slim-right\" ng-if=\" i == 1 \" rowspan=\"{{ $ctrl.getNumberEdgeTo().length }}\">Х</td>\n\t\t\t\n\t\t\t<td class=\"slim-left slim-right\">\n\t\t\t\t&Delta; <sub>{{i}}</sub>\n\t\t\t</td>\n\t\t\t\n\t\t\t<td class=\"slim-left slim-right\" ng-if=\" i == 1 \" rowspan=\"{{ $ctrl.getNumberEdgeTo().length }}\">=</td>\n\n\t\t\t<td>\n\t\t\t\t{{$ctrl.Q[i-1]}}\n\t\t\t</td>\n\n\t\t</tr>\n\t</table>\n\n\t<table>\n\t\t<tr ng-repeat=\"i in $ctrl.getNumberEdgeTo() track by $index\">\n\t\t\t<td ng-repeat=\"j in $ctrl.getNumberEdgeTo() track by $index\">\n\t\t\t\t{{$ctrl.A[i-1][j-1]}} х &Delta;<sub>{{j}}</sub> <span ng-if=\"!(j == $ctrl.getNumberEdgeTo().length)\">+</span>\n\t\t\t</td>\t\t\t\n\t\t\t<td>=</td>\n\t\t\t<td>\n\t\t\t\t{{$ctrl.Q[i-1]}}\n\t\t\t</td>\n\t\t</tr>\n\t</table>\n\t\n\t<br>\n\t<p>Глобальный вектор перемещений:</p>\n\n\t<span ng-repeat=\"i in $ctrl.getNumberEdgeTo() track by $index\">\n\t\t\t&Delta;<sub>{{i}}</sub> = \n\t\t\t{{ $ctrl.gaussRound($ctrl.resultDelta[i-1], 4) }}\n\t\t\t<br>\n\t</span>\t\t\t\n\n\n</div>";

/***/ },
/* 20 */
/***/ function(module, exports) {

	'use strict';Object.defineProperty(exports,"__esModule",{value:true});exports.default=function(){var service={start:false,// запущена ли отрисовка
	minimal:false,// масштабирование с коэффициент 0
	ye:10,// условные единицы
	kq:1,kF:1,base:{A:100,// A
	L:10,// L
	E:10e5.toExponential(),sigma:'',// допускаемое напряжене
	q:false},structure:{item:[],// KernelList
	F:[],// PowerFList
	leftSealing:false,// заделка слева
	rightSealing:false// заделка
	},spc_version:1.0};return service;};;
...
/***/ function(module, exports) {
	...
	'use strict';(function(){var elif=angular.module('elif',[]);var CONDITIONALS='elif.conditionals';// This is copied from AngularJS because it is not
	// part of the public interface.
	var getBlockElements=function getBlockElements(nodes){if(!nodes||!nodes.length){return angular.element();}var startNode=nodes[0];var endNode=nodes[nodes.length-1];if(startNode===endNode){return angular.element(startNode);}var element=startNode;var elements=[element];do{element=element.nextSibling;if(!element){break;}elements.push(element);}while(element!==endNode);return angular.element(elements);};elif.factory('elif',[function(){// By requiring the scope have it's own copy of `elif.conditionals`
	// we avoid if/else-if/else structures that span AngularJS scopes.
	var getConditionals=function getConditionals(scope){if(angular.hasOwnProperty.call(scope,CONDITIONALS)){var conditionals=scope[CONDITIONALS];return conditionals[conditionals.length-1];}};return{create:function create(scope,fn,callback){var conditionals=[{fn:fn,callback:callback||angular.identity}];var conditionalValues=[];scope.$watch(function(){// Watch the boolean conditionals; we only need
	...
	// condition.
	var i;var len=conditionals.length;conditionalValues.length=len;for(i=0;i<len;i++){if(conditionalValues[i]=!!conditionals[i].fn()){i++;break;}}for(;i<len;i++){conditionalValues[i]=false;}return conditionalValues;},function(conditionalValues){// Update each block; also find first matching if/else-if.
	var index=-1;for(var i=0,len=conditionals.length;i<len;i++){if(conditionalValues[i]){conditionals[i].callback(true);index=i;}else{conditionals[i].callback(false);}}// Handle else, if there is one.
	conditionals.fallthrough&&conditionals.fallthrough(index===-1);},true);// Deep watch; we know that it is a simple list of booleans.
	// Keep track of if/else-if/else structures per AngularJS scope.
	if(!angular.hasOwnProperty.call(scope,CONDITIONALS)){scope[CONDITIONALS]=[];}scope[CONDITIONALS].push(conditionals);},extend:function extend(scope,fn,callback){var conditionals=getConditionals(scope);if(!conditionals){throw new Error('elif.extend: no if found at this level');}if(conditionals.fallthrough){throw new Error('elif.extend: else-if after else');}conditionals.push({fn:fn,callback:callback});},fallthrough:function fallthrough(scope,fn,callback){var conditionals=getConditionals(scope);if(!conditionals){throw new Error('elif.fallthrough: no if found at this level');}if(conditionals.fallthrough){throw new Error('elif.fallthrough: else already found at this level');}conditionals.fallthrough=callback;}};}]);// This implementation is basically the built-in `ng-if`, hooked into the `elif` service.
	var elifDirective=function elifDirective(name,method,getter){elif.directive(name,['$animate','$document','$injector','elif',function($animate,$document,$injector,elif){var getterFn=getter&&$injector.invoke(getter);return{transclude:'element',restrict:'A',priority:600,terminal:true,link:function link(scope,element,attrs,ctrls,transcludeFn){var watchFn=getterFn&&getterFn(scope,element,attrs);var childScope;var childElement;var previousElements;elif[method](scope,watchFn,function(value){if(value){if(!childScope){childScope=scope.$new();transcludeFn(childScope,function(clone){clone[clone.length+1]=$document[0].createComment(' end '+name+': '+attrs[name]+' ');childElement=clone;$animate.enter(clone,element.parent(),element);});}}else{if(childScope){childScope.$destroy();childScope=null;}if(previousElements){previousElements.remove();previousElements=null;}if(childElement){previousElements=getBlockElements(childElement);$animate.leave(previousElements,function(){previousElements=null;});childElement=null;}}});}};}]);};// Reads the attribute given by `name` and converts it to a boolean.
	var getter=function getter(name){return['$parse',function($parse){return function(scope,element,attrs){var testFn=$parse(attrs[name]);return function(){return!!testFn(scope);};};}];};// We rely on the built-in `ng-if` directive to actually perform
	// the transclusion, and simply tie it in to the `elif` service.
	elif.directive('ngIf',['$injector','elif',function($injector,elif){var getterFn=$injector.invoke(getter('ngIf'));return{priority:600,link:function link(scope,element,attrs){var watchFn=getterFn(scope,element,attrs);elif.create(scope,watchFn);}};}]);// Else-if and else perform their own transclusions.
	elifDirective('ngElseIf','extend',getter('ngElseIf'));elifDirective('ngElif','extend',getter('ngElif'));// Else doesn't take an argument.
...
/***/ function(module, exports) {
	...
	"use strict";var abs=Math.abs;function array_fill(i,n,v){var a=[];for(;i<n;i++){a.push(v);}return a;}/**
	 * Gaussian elimination
	 * @param  array A matrix
	 * @param  array x vector
	 * @return array x solution vector
	 */function gauss(A,x){var i,k,j;// Just make a single matrix
	for(i=0;i<A.length;i++){A[i].push(x[i]);}var n=A.length;for(i=0;i<n;i++){// Search for maximum in this column
	var maxEl=abs(A[i][i]),maxRow=i;for(k=i+1;k<n;k++){if(abs(A[k][i])>maxEl){maxEl=abs(A[k][i]);maxRow=k;}}// Swap maximum row with current row (column by column)
	for(k=i;k<n+1;k++){var tmp=A[maxRow][k];A[maxRow][k]=A[i][k];A[i][k]=tmp;}// Make all rows below this one 0 in current column
	for(k=i+1;k<n;k++){var c=-A[k][i]/A[i][i];for(j=i;j<n+1;j++){if(i===j){A[k][j]=0;}else{A[k][j]+=c*A[i][j];}}}}// Solve equation Ax=b for an upper triangular matrix A
	x=array_fill(0,n,0);for(i=n-1;i>-1;i--){x[i]=A[i][n]/A[i][i];for(k=i-1;k>-1;k--){A[k][n]-=A[k][i]*x[i];}}return x;}module.exports=gauss;
